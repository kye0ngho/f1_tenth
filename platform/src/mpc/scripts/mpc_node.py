...
class mpc_config:
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([100.0, 100.0, 0.0, 0.0])  # levine sim
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.is_real = False
        self.map_name = 'levine_full'

        # create ROS subscribers and publishers
        pose_topic = "/pf/viz/inferred_pose" # if self.is_real else "/ego_racecar/odom"
        drive_topic = "/sim/drive"
        vis_ref_traj_topic = "/ref_traj_marker"
        vis_waypoints_topic = "/waypoints_marker"
        vis_pred_path_topic = "/pred_path_marker"

        self.pose_sub = self.create_subscription(PoseStamped if not self.is_real else Odometry, pose_topic, self.pose_callback, 1)
        self.pose_sub  # prevent unused variable warning

        self.drive_pub = self.create_publisher(AckermannDriveStamped, drive_topic, QoSProfile(depth=1, reliability=1))
        ...
        map_path = os.path.abspath(os.path.join('src', 'mpc'))
        self.waypoints = np.loadtxt(map_path + '/racelines/' + self.map_name + '.csv', delimiter=';', skiprows=0)  # csv data
    ...
    def update_rotation_matrix(self, pose_msg):
        ...
        curr_orien = pose_msg.pose.orientation if not self.is_real else pose_msg.pose.pose.orientation
    ...
    def update_vehicle_state(self, pose_msg):
        ...
        """
        vehicle_state = State()
        vehicle_state.x = pose_msg.pose.position.x if not self.is_real else pose_msg.pose.pose.position.x
        vehicle_state.y = pose_msg.pose.position.y if not self.is_real else pose_msg.pose.pose.position.y
