...
def _launch_setup(context):
    ...
    if controller in pure_pursuit_family:
        ...
        if controller == 'racing_v1_pp' and drive_mode == 'sim':
            # v1 is meant to stay a frozen, real-car-safe reference
            # (2.6 m/s^2 lateral, its own code default -- see the "NOT
            # wired" comment above). This override only ever applies in
            # drive_mode:=sim, and only to racing_v1_pp: v2/v3 already
            # carry their own real-validated tuning as code defaults and
            # must not be touched here.
            #
            # Values from a lap-time sweep on the busan sim loop
            # (num_agent:1, no obstacle, ~44m, target_speed 10 m/s).
            # max_lateral_acceleration/max_longitudinal_acceleration/
            # max_longitudinal_deceleration were each swept independently
            # first (best-in-isolation: 20 / 12 / 15), but that combination
            # ran the car off the track within one lap -- per-parameter
            # margins do not add, so the full grid of individually-safe
            # values around that point was tried together instead:
            #   lat / accel / decel -> best lap (40-90s run)
            #    18 /   10   /  10  -> 8.39s (9 laps / 80s)
            #    20 /   10   /  10  -> 8.33s (4 laps / 40s)
            #    18 /   12   /  12  -> 8.32s (4 laps / 40s)
            #    18 /   10   /  12  -> 8.37s (4 laps / 40s)
            #    20 /   12   /  15  -> STUCK (ran off track, lap 2)
            #    20 /   10   /  12  -> 8.17s (10 laps / 90s)  <- selected
            # Not validated on real hardware.
            pp_parameters.append({
                'max_lateral_acceleration': 20.0,
                'max_longitudinal_acceleration': 10.0,
                'max_longitudinal_deceleration': 12.0,
                'corner_slowdown_gain': 0.0,
            })
        return [
            LogInfo(msg=(
