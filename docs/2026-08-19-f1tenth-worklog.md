...
**다음 세션 시작점(가장 중요, 여기부터 읽을 것)**: 이 문서 맨 아래
"## 2026-08-19 후속 (같은 날 연장 세션)" 섹션. 커밋 `35cae27`(WIP, 미검증)로
끝났고, `_debounce`의 freeze-at-commit 메커니즘 재설계가 최우선 과제.
메모리 `project_kkh_occupancy_grid_avoidance.md`에도 같은 내용 요약돼있음.
